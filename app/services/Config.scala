...
class ConfigBuilder(val aws: AWS) extends AwsInstanceTags {

  lazy val conf = readTag("Stage") match {
    case Some("PROD") => new ProdConfig(aws)
    case Some("CODE") => new CodeConfig(aws)
    case _            => new DevConfig(aws)
  }
}

sealed trait Config extends AwsInstanceTags {
  ...
  lazy val pandaHMACSecret: String = getRequiredRemoteStringProperty("panda.hmac.secret")
  ...
  lazy val dfpClientId: String     = getRequiredRemoteStringProperty("dfp.client.id")
  lazy val dfpClientSecret: String = getRequiredRemoteStringProperty("dfp.client.secret")
  lazy val dfpRefreshToken: String = getRequiredRemoteStringProperty("dfp.refresh.token")
...
class DevConfig(val aws: AWS) extends Config {
  ...
  override val dfpNetworkCode: String                           = StagingDfpProperties.dfpNetworkCode
  override val dfpNativeCardOrderIds: Map[String, Seq[Long]]    = StagingDfpProperties.dfpNativeCardOrderIds
  override val dfpMerchandisingOrderIds: Map[String, Seq[Long]] = StagingDfpProperties.dfpMerchandisingOrderIds
  override val dfpCampaignFieldId: Long                         = StagingDfpProperties.dfpCampaignFieldId
}

class CodeConfig(val aws: AWS) extends Config {
...
class ProdConfig(val aws: AWS) extends Config {
  override def stage = "PROD"
  ...
  override val dfpNativeCardOrderIds: Map[String, Seq[Long]] = {
  ...
  override val dfpMerchandisingOrderIds: Map[String, Seq[Long]] = {
