...
object Dfp extends DfpService {
    ...
    val fetches: Stream[Seq[LineItem]] = {
      ...
      def fetch(nameCondition: String): Stream[Seq[LineItem]] = {
        ...
        def isAlreadyLinked(item: LineItem): Boolean = {
          safeSeq(item.getCustomFieldValues) exists (_.getCustomFieldId == config.dfpCampaignFieldId)
        }
  ...
  def getCampaignIdCustomField(config: Config, lineItem: LineItem): Option[BaseCustomFieldValue] =
    safeSeq(lineItem.getCustomFieldValues) find (_.getCustomFieldId == config.dfpCampaignFieldId)

  def getCampaignIdCustomFieldValue(config: Config, lineItem: LineItem): Option[String] = {
    getCampaignIdCustomField(config, lineItem) map {
      _.asInstanceOf[CustomFieldValue].getValue.asInstanceOf[TextValue].getValue
    }
  }

  def hasCampaignIdCustomFieldValue(config: Config, campaignId: String)(lineItem: LineItem): Boolean =
    getCampaignIdCustomFieldValue(config, lineItem) contains campaignId

  def linkLineItemToCampaign(config: Config,
                             service: LineItemServiceInterface,
                             lineItemId: Long,
                             campaignId: String): Unit = {
    fetchLineItemById(service, lineItemId) foreach { item =>
      val currCampaignId = getCampaignIdCustomFieldValue(config, item)
      if (currCampaignId.isDefined) {
        Logger.error(s"Line item $lineItemId is already linked to campaign ${currCampaignId.get}")
      } else {
        appendCustomFieldToLineItem(service,
                                    item,
                                    new CustomFieldValue(config.dfpCampaignFieldId, new TextValue(campaignId)))
      }
    }
  }
...
sealed trait DfpService {
  ...
  def mkSession(config: Config): DfpSession = {
    new DfpSession.Builder()
      .withOAuth2Credential(
        new OfflineCredentials.Builder()
          .forApi(DFP)
          .withClientSecrets(config.dfpClientId, config.dfpClientSecret)
          .withRefreshToken(config.dfpRefreshToken)
          .build()
          .generateCredential()
      )
      .withApplicationName(config.dfpAppName)
      .withNetworkCode(config.dfpNetworkCode)
      .build()
  }
  ...
  def fetchReport(service: ReportServiceInterface, qry: ReportQuery): Try[BufferedSource] = {
    ...
    Try(reportDownloader.waitForReportReady()) map { _ =>
      val source = {
