...
object QualifiedMetricReport {
  implicit val qualifiedMetricReportFormat: Format[QualifiedMetricReport] =
    Jsonx.formatCaseClass[QualifiedMetricReport]
}
...
case class ContentTypeDwellTimeMetric(contentType: String, qualifiedDwellTime: Int)
  ...
  override def fetch(campaign: Campaign,
                     startDate: DateTime,
                     endDate: Option[DateTime]): Map[String, QualifiedMetricReport] = {
    ...
    val report = campaignFilter.map { filter =>
      ...
      if (totalHits > 0) {
        val qualifiedHits = GoogleAnalytics.loadCampaignContentTypeViewsWithDwellTime(filter,
                                                                                      contentType,
                                                                                      qualifiedDwellTime,
                                                                                      startDate,
                                                                                      endDate)
        Map(
          s"${contentType}DwellTime" -> (QualifiedMetricReport(totalHits,
                                                               qualifiedHits,
